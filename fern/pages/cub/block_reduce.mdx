...
```cpp
cub::BlockReduce< T, BlockDimX, Algorithm, BlockDimY, BlockDimZ >::BlockReduce(TempStorage &temp_storage)
```
...
|---|---|---|
| `BLOCK_THREADS` <Badge intent="info" minimal>static</Badge> <Badge intent="info" minimal>constexpr</Badge> | `int` | The thread block size in threads. |
